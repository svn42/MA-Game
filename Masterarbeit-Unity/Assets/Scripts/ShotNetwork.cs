...
public class ShotNetwork : NetworkBehaviour
{
    ...
    void Start()
    {
        ...
        {
            if (playerList[i].GetComponent<PlayerNetwork>().playerTeam == playerTeam)
            {
                player = playerList[i];
    ...
    public void OnCollisionEnter2D(Collision2D coll)
    {
        ...
        {
            case "Block":
                collidingObject.GetComponent<BlockNetwork>().ReduceHealth(strength);
                DestroyShot();
                playerLogging.AddAccuracy("block");
                break;
            case "Player":
                if (collidingObject.GetComponent<PlayerNetwork>().playerTeam != playerTeam)   //wenn der Schuss den gegnerischen Spieler trifft
                {
            ...
            case "Boundary":
                ...
                break;
            case "Shot":
                if (collidingObject.GetComponent<ShotNetwork>().GetPlayerTeam() != playerTeam)
                {
                    gameState.PlaySound("soundSlap", 0.05f);

                    if (strength < collidingObject.GetComponent<ShotNetwork>().strength)
                    {
                        DestroyShot();
                    }
                    else if (strength == collidingObject.GetComponent<ShotNetwork>().strength)
                    {
                ...
                {
                    if (shotID > collidingObject.GetComponent<ShotNetwork>().GetShotID())
                    {
                        DestroyShot();
                        playerLogging.AddAccuracy("destroy");
                    }
            ...
            case "Ball":
                ...
                collidingObject.GetComponent<BallNetwork>().SetLastHitBy(playerTeam);
                switch (strength)
                {
                    case 1:
                        collidingObject.GetComponent<BallNetwork>().PlayHitSound(0.3f);
                        break;
                    case 2:
                        collidingObject.GetComponent<BallNetwork>().PlayHitSound(0.5f);
                        break;
                    case 3:
                        collidingObject.GetComponent<BallNetwork>().PlayHitSound(0.8f);
                        break;
                }
                DestroyShot();

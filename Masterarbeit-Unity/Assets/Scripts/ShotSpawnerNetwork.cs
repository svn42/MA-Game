...
public class ShotSpawnerNetwork : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        player = transform.parent.GetComponent<PlayerNetwork>();
    ...
    {
        shotCount++;
        shot.GetComponent<ShotNetwork>().SetDirection(this.transform.rotation);    //Der Schuss bekommt die Rotation des Spielers übergeben
        shot.GetComponent<ShotNetwork>().SetColor(shotColor);                      //dessen Farbe
        shot.GetComponent<ShotNetwork>().SetPlayerTeam(playerTeam);                //dessen Team
        shot.GetComponent<ShotNetwork>().SetShotID(shotCount);                     //sowie seine ID
